...
class TelegramBot:
    _logger: Logger
    __user_ids: list[int]
    __types = ["_command_handler", "_message_handler"]
            ...
            for handler in handler_functions:
                if _type == "_message_handler":
                    self.application.add_handler(
                        MessageHandler(
                            filters.TEXT & ~filters.COMMAND & filters.Chat(self.__user_ids), getattr(self, handler)
                        )
                    )
                    ...
                    self.application.add_handler(
                        CommandHandler(
                            handler.replace(_type, ""), getattr(self, handler), filters.Chat(self.__user_ids)
                        )
                    )

    async def _error_handler(self, update: object | None, context: CallbackContext) -> None:
